...
class Ranker:
	...
	def build_model(self):
		...
		with tf.variable_scope('bookkeeping') as scope:
			#对历史的对话记录进行累积
			self.user_weight =  tf.Variable(tf.random_uniform([self.memory_size,self.book_keep_size],-0.1,0.1),name = 'user_merge')
			self.zero_weight =  tf.Variable(tf.zeros([self.memory_size,self.book_keep_size]),name = 'zero_weight',trainable=False)#全0权值矩阵
			self.user_bias = tf.Variable(tf.zeros([self.book_keep_size]),name = 'user_bias')
			self.host_weight = self.user_weight	#使用公共的变量
			self.host_bias = self.user_bias	#共享变量，优化比较好
			self.user_book=[]
			self.host_book=[]
			def inner_linear(iinput, iweight, ibias):
				ioutput = tf.matmul(iinput,iweight)
				if ibias!=None:
					ioutput = tf.add(ioutput,ibias)
				return ioutput
			self.user_book.append( inner_linear(self.history_out[0], self.user_weight, self.user_bias ))
			#用来累计用户说了哪些话
			self.host_book.append( inner_linear(self.history_out[1], self.zero_weight, None))
			#用来累计host说了哪些话
			for i in range(1,len(self.history_out)):
				if i%2==0:#user
					temp = inner_linear(self.history_out[i], self.user_weight, self.user_bias)
					self.user_book.append(tf.add(self.user_book[-1], temp))
				else:	#host
					temp = inner_linear(self.history_out[i],self. host_weight, self.host_bias)
					self.host_book.append(tf.add(self.host_book[-1], temp))
		...
		with tf.variable_scope("dialog_rnn") as scope:
			...
			self.merge_weight = tf.Variable(tf.random_uniform([(self.memory_size+self.book_keep_size)*2,self.memory_size],-0.1,0.1),name = 'merge')
...
#			tf.histogram_summary('merge_weight',self.merge_weight)	#统计合并权重的分布
			...
			for i in range(len(self.context_out)):
				if i%2==0:	#只有需要进行预测的时候进行状态合并
					...
					concat_state = tf.concat(1,[self.context_out[i],self.history_out[i],tf.tanh(self.user_book[int(i/2)]),tf.tanh(self.host_book[int(i/2)])])	#state merge
